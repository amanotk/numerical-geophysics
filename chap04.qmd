...
Euler方程式では中間固有値がエントロピーモードに対応することを考えると，接触不連続面の性質から$v_{*L} = v_{*R} = S_*$, $p_{*L} = p_{*R}$と仮定してもよいだろう．このとき，式$(\ref{eq:conservation-law})$を用いて以下を得る．
$$
...
$$
とコンパクトに書けることが分かる．ただしここで$\bm{D}_{*} = (0, 1, S_*)$である．
これを使えば数値流速は以下のように求まる．
$$
\bm{f}_{*K} =
\frac{
