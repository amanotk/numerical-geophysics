...
- Lax-Wendroffの定理：保存形の数値計算スキームを考えるとき，数値解が収束するならば，元の微分方程式の弱解に収束する．（ただし膨張衝撃波も弱解の1つなので，収束する解が物理的な解とは限らないことに注意が必要である．） 非保存形のスキームは一般に衝撃波速度を正しく与えない．したがって，衝撃波（やそれに類する不連続面）が重要になる問題では保存形スキームがほぼ必須となっている．
